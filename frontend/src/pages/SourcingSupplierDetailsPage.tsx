          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            {!campaign ? (
              ...
              <div className="space-y-3">
                ...
                {/* Actions — reply detected, awaiting manual convert */}
                {canEdit && campaign.status === "response_received" && supplier.status !== "converted_to_new" && (
                  <div className="space-y-2 pt-2 border-t border-slate-100">
                    <p className="text-xs text-emerald-700 font-medium">Supplier replied — ready to convert</p>
                    <Button size="sm" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white" onClick={() => markResponseMutation.mutate()} disabled={markResponseMutation.isPending}>
                      {markResponseMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-1.5" /> : <CheckCircle2 className="h-4 w-4 mr-1.5" />}
