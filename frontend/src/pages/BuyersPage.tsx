...
const DOCUMENT_TYPES = [
  "NDA", "Contract", "Compliance Doc", "Certificate",
  "Invoice", "Quotation", "Specification Sheet", "Other",
] as const;
...
export default function BuyersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const uploadPendingDocs = async (buyerId: string) => {
      for (const { file, docType } of pendingDocFiles) {
        const fd = new FormData();
        fd.append("file", file);
        fd.append("documentType", docType);
        await api.post(`/buyers/${buyerId}/documents`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
      setPendingDocFiles([]);
    ...
    if (editingBuyer?.id) {
      try {
        await updateMutation.mutateAsync({ id: editingBuyer.id, data: payload });
        if (pendingDocFiles.length > 0) await uploadPendingDocs(editingBuyer.id);
      } catch { /* handled by mutation */ }
    } else {
      try {
        const res = await createMutation.mutateAsync(payload);
        const newId = res.data?.id;
        if (pendingDocFiles.length > 0 && newId) await uploadPendingDocs(newId);
      } catch { /* handled by mutation */ }
    }
  };
        ...
        <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto p-6 bg-white rounded-xl shadow-2xl border-none">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 mt-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2"><Label>Phone (with country code) *</Label><Input value={form.phone ?? ""} onChange={(e) => setForm({ ...form, phone: e.target.value })} required /></div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {(form.additionalContacts || []).map((c, i) => (
                        <tr key={i} className={(!c.phone || !c.whatsapp) ? "bg-amber-50/40" : ""}>
                          ...
                          <td className="px-2 py-1.5"><Input className={`h-7 text-xs border-slate-200 ${!c.phone ? "border-amber-300 bg-amber-50" : ""}`} value={c.phone} onChange={(e) => updateContact(i, "phone", e.target.value)} /></td>
                          <td className="px-2 py-1.5"><Input className={`h-7 text-xs border-slate-200 ${!c.whatsapp ? "border-amber-300 bg-amber-50" : ""}`} value={c.whatsapp} onChange={(e) => updateContact(i, "whatsapp", e.target.value)} /></td>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2"><Label>Relationship Tier</Label><Select value={form.relationshipTier ?? ""} onValueChange={(v) => setForm({ ...form, relationshipTier: v === "_clear" ? "" : v })}><SelectTrigger><SelectValue placeholder="Select tier…" /></SelectTrigger><SelectContent><SelectItem value="_clear"><span className="text-muted-foreground">— None —</span></SelectItem><SelectItem value="Transactional">Transactional</SelectItem><SelectItem value="Active">Active</SelectItem><SelectItem value="Strategic">Strategic</SelectItem></SelectContent></Select></div>
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-3">Commercial Intelligence</p>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label>Initial Order Value</Label>
                  <Input value={form.initialOrderValue ?? ""} onChange={(e) => setForm({ ...form, initialOrderValue: e.target.value })} placeholder="e.g. USD 25,000" />
                </div>
                <div className="space-y-2">
                  <Label>Potential Annual Value</Label>
                  <Input value={form.potentialAnnualValue ?? ""} onChange={(e) => setForm({ ...form, potentialAnnualValue: e.target.value })} placeholder="e.g. USD 200,000" />
                </div>
                <div className="space-y-2">
                  <Label>Reorder Likelihood</Label>
                  <Select value={form.reorderLikelihood ?? ""} onValueChange={(v) => setForm({ ...form, reorderLikelihood: v === "_clear" ? "" : v })}>
                ...
                <div className="space-y-2 sm:col-span-2">
                  ...
                  <Input value={form.buyerStatedIntent ?? ""} onChange={(e) => setForm({ ...form, buyerStatedIntent: e.target.value })} placeholder="e.g. Plans to place monthly orders of 20 MT if first shipment is approved" />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {/* Existing uploaded docs (edit mode) */}
                {editingBuyer?.id && (form as Partial<Buyer> & { documents?: BuyerDocument[] }).documents?.map((doc) => (
