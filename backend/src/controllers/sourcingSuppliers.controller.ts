    ...
    try {
        ...
        const [suppliers, total] = await Promise.all([
            (prisma as any).sourcingSupplier.findMany({
                ...
                include: { emailCampaign: true, creator: { select: { fullName: true, email: true } } },
            }),
 ...
 */
export async function getSourcingCategoryStats(_req: AuthRequest, res: Response): Promise<void> {
    try {
 ...
 */
export async function bulkCreateSourcingSuppliers(req: AuthRequest, res: Response): Promise<void> {
    try {
        ...
        const data = validRows.map((s) => {
            ...
            if (s.linkedinUrl?.trim()) notesParts.push(`LinkedIn: ${s.linkedinUrl.trim()}`);
 ...
 */
export async function getSourcingCreators(req: AuthRequest, res: Response): Promise<void> {
    try {
