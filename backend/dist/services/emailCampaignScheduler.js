...
async function sendDailyFollowupReminders() {
    try {
        ...
        await Promise.allSettled(activeAdmins.map((r) => sendFollowupReminderEmail({
            to: r.user.email,
            adminName: r.user.fullName,
            suppliers,
        })));
        // Create in-app notifications for each due campaign
        await Promise.allSettled(dueCampaigns.map((c) => createNotification({
        ...
        console.log(`[EmailCampaignScheduler] Sent follow-up reminders to ${activeAdmins.length} admin(s) for ${suppliers.length} supplier(s).`);
    }
    catch (err) {
...
//# sourceMappingURL=emailCampaignScheduler.js.map