...
export async function listBuyers(req, res) {
    try {
        const { search = "", page = "1", limit = "20", status, product, } = req.query;
 ...
 */
export async function getBuyerStats(_req, res) {
    try {
 ...
 */
export async function getCrProducts(_req, res) {
    try {
        ...
        for (const buyer of buyers) {
            const products = buyer.requiredProducts;
 ...
 */
export async function exportBuyersCsv(req, res) {
    try {
        ...
        const rows = buyers.map((b) => [
            ...
            Array.isArray(b.requiredProducts)
                ? b.requiredProducts.map((p) => p.name).join("; ")
                : "",
            b.notes || "",
        ...
        res.setHeader("Content-Disposition", `attachment; filename=buyers_export_${new Date().toISOString().split("T")[0]}.csv`);
        res.send(csvContent);
    }
    catch (err) {
